    ...
    public class Event2026 : ControllerBase
    {
        ...
        public async Task<ActionResult<List<EventDTO>>> PostEvent(RRScout.Entities.Event eventDTO)
        {
            try
            {
                ...
                newEvent.folderID = eventDTO.folderID;
        ...
        //[Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        public async Task<ActionResult<EventDTO>> getEventInfo(string eventID)
        {
            var eventInfo = await Context.Events.Where(x => x.eventCode == eventID).SingleOrDefaultAsync();
            if (eventInfo != null)
            {
                return Ok(eventInfo);
            }
